class NcosController < ApplicationController
  ...
  # POST /boats.json
  def create
    @nomination = Nco.new(nomination_params)
    @info = NominationType.where(["code = ?", "Nco"]).first
    ...
    respond_to do |format|
      if @nomination.save
        ...
        format.html { redirect_to '/Ncos/confirmation', :layout => "nomination_form", notice: 'Nomination was successfully created.' }
  ...
  private
    ...
    def nomination_type
        Nomination.nomination_types.include?(params[:type]) ? params[:type] : "Nco"
    end
