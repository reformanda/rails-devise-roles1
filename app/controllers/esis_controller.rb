class EsisController < ApplicationController
  ...
  # POST /boats.json
  def create
    @nomination = Esi.new(nomination_params)
    @info = NominationType.where(["code = ?", "Esi"]).first
    ...
    respond_to do |format|
      if @nomination.save
        ...
        format.html { redirect_to '/Esis/confirmation', :layout => "nomination_form", notice: 'Nomination was successfully created.' }
  ...
  private
    ...
    def nomination_type
        Nomination.nomination_types.include?(params[:type]) ? params[:type] : "Esi"
    end
