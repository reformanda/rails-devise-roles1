class SaecSpecsController < ApplicationController
  ...
  # GET /boats/1.json
  def confirmation
    @info = NominationType.where(["code = ?", "SaecSpec"]).first
  end

  # GET /boats/new
  def new
    @nomination = SaecSpec.new
    @info = NominationType.where(["code = ?", "SaecSpec"]).first
    @award_options = AwardOption.joins(:nomination_type).where("code  = ?", "SaecSpec").order("award_options.name").pluck(:name,:id)
  ...
  # POST /boats.json
  def create
    @nomination = SaecSpec.new(nomination_params)
    @info = NominationType.where(["code = ?", "SaecSpec"]).first
    ...
    respond_to do |format|
      if @nomination.save
        ...
        format.html { redirect_to '/saec_specs/confirmation', :layout => "nomination_form", notice: 'Nomination was successfully created.' }
        format.json { render :confirmation, status: :created, location: @nomination }
      else
        @award_options = AwardOption.joins(:nomination_type).where("code  = ?", "SaecSpec").pluck(:name,:id)
        @callback = "/saec_specs/?#no-back"
  ...
  private
    ...
    def set_nomination
      @saec_spec = SaecSpec.find(params[:id])
    end
    ...
    def nomination_type
        Nomination.nomination_types.include?(params[:type]) ? params[:type] : "SaecSpec"
    end
    ...
    def nomination_params
      params.require(:saec_spec).permit(
