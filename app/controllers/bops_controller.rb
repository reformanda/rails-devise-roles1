class BopsController < ApplicationController
  ...
  # GET /boats/1.json
  def confirmation
    @info = NominationType.where(["code = ?", "Bop"]).first
  end
  ...
  def new
    @nomination = Bop.new
    @info = NominationType.where(["code = ?", "Bop"]).first
    @award_options = AwardOption.joins(:nomination_type).where("code  = ?", "Bop").pluck(:name,:id)
    @callback = "/bops/?#no-back"
    begin
  ...
  # POST /boats.json
  def create
    @nomination = Bop.new(nomination_params)
    @info = NominationType.where(["code = ?", "Bop"]).first
    ...
    respond_to do |format|
      if @nomination.save
        ...
        format.html { redirect_to '/bops/confirmation', :layout => "nomination_form", notice: 'Nomination was successfully created.' }
  ...
  private
    ...
    def nomination_type
        Nomination.nomination_types.include?(params[:type]) ? params[:type] : "Bop"
    end
