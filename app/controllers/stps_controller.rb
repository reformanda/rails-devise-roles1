class StpsController < ApplicationController
  ...
  # GET /boats/1.json
  def confirmation
    @info = NominationType.where(["code = ?", "Stp"]).first
  end
  ...
  def new
    @nomination = Stp.new
    @info = NominationType.where(["code = ?", "Stp"]).first
    @award_options = AwardOption.joins(:nomination_type).where("code  = ?", "Stp").pluck(:name,:id)
    @callback = "/stps/?#no-back"
    begin
  ...
  # POST /boats.json
  def create
    @nomination = Stp.new(nomination_params)
    @info = NominationType.where(["code = ?", "Stp"]).first
    ...
    respond_to do |format|
      if @nomination.save
        ...
        format.html { redirect_to '/stps/confirmation', :layout => "nomination_form", notice: 'Nomination was successfully created.' }
  ...
  private
    ...
    def nomination_type
        Nomination.nomination_types.include?(params[:type]) ? params[:type] : "Stp"
    end
