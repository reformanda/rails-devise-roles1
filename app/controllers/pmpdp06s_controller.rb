class Pmpdp06sController < ApplicationController
  ...
  # POST /boats.json
  def create
    @nomination = Pmpdp06.new(nomination_params)
    @info = NominationType.where(["code = ?", "Pmpdp06"]).first
    ...
    respond_to do |format|
      if @nomination.save
        ...
        format.html { redirect_to '/Pmpdp06s/confirmation', :layout => "nomination_form", notice: 'Nomination was successfully created.' }
  ...
  private
    ...
    def set_nomination
      @Pmpdp06 = Pmpdp06.find(params[:id])
    end
    ...
    def nomination_type
        Nomination.nomination_types.include?(params[:type]) ? params[:type] : "Pmpdp06"
    end
