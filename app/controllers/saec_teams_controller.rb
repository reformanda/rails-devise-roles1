class SaecTeamsController < ApplicationController
  ...
  # GET /boats/1.json
  def confirmation
    @info = NominationType.where(["code = ?", "SaecTeam"]).first
  end

  # GET /boats/new
  def new
    @nomination = SaecTeam.new
    @info = NominationType.where(["code = ?", "SaecTeam"]).first
    @award_options = AwardOption.joins(:nomination_type).where("code  = ?", "SaecTeam").order("award_options.name").pluck(:name,:id)
    #session[:award_options] = [["Program Management",1]]
    @callback = "/saec_teams/?#no-back"
    begin
  ...
  # POST /boats.json
  def create
    @nomination = SaecTeam.new(nomination_params)
    @info = NominationType.where(["code = ?", "SaecTeam"]).first
    ...
    respond_to do |format|
      if @nomination.save
        ...
        format.html { redirect_to '/saec_teams/confirmation', :layout => "nomination_form", notice: 'Nomination was successfully created.' }
        format.json { render :confirmation, status: :created, location: @nomination }
      else
        @award_options = AwardOption.joins(:nomination_type).where("code  = ?", "SaecTeam").pluck(:name,:id)
        @callback = "/saec_teams/?#no-back"
  ...
  private
    ...
    def nomination_type
        Nomination.nomination_types.include?(params[:type]) ? params[:type] : "SaecTeam"
    end
