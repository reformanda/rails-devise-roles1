class SaecOffsController < ApplicationController
  ...
  # GET /boats/1.json
  def confirmation
    @info = NominationType.where(["code = ?", "SaecOff"]).first
  end

  # GET /boats/new
  def new
    @nomination = SaecOff.new
    @info = NominationType.where(["code = ?", "SaecOff"]).first
    @award_options = AwardOption.joins(:nomination_type).where("code  = ?", "SaecOff").order("award_options.name").pluck(:name,:id)
    #session[:award_options] = [["Program Management",1]]
    @callback = "/saec_offs/?#no-back"
    begin
  ...
  # POST /boats.json
  def create
    @nomination = SaecOff.new(nomination_params)
    @info = NominationType.where(["code = ?", "SaecOff"]).first
    ...
    respond_to do |format|
      if @nomination.save
        ...
        format.html { redirect_to '/saec_offs/confirmation', :layout => "nomination_form", notice: 'Nomination was successfully created.' }
        format.json { render :confirmation, status: :created, location: @nomination }
      else
        @award_options = AwardOption.joins(:nomination_type).where("code  = ?", "SaecOff").pluck(:name,:id)
        @callback = "/saec_offs/?#no-back"
  ...
  private
    ...
    def set_nomination
      @saec_spec = SaecOff.find(params[:id])
    end
    ...
    def nomination_type
        Nomination.nomination_types.include?(params[:type]) ? params[:type] : "SaecOff"
    end
    ...
    def nomination_params
      params.require(:saec_off).permit(
