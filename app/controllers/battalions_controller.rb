class BattalionsController < ApplicationController
  ...
  # POST /boats.json
  def create
    @nomination = Battalion.new(nomination_params)
    @info = NominationType.where(["code = ?", "Battalion"]).first
    ...
    respond_to do |format|
      if @nomination.save
        ...
        format.html { redirect_to '/battalions/confirmation', :layout => "nomination_form", notice: 'Nomination was successfully created.' }
  ...
  private
    ...
    def nomination_type
        Nomination.nomination_types.include?(params[:type]) ? params[:type] : "Battalion"
    end
    ...
    def nomination_params
      params.require(:battalion).permit(
