class Pmpdp05sController < ApplicationController
  ...
  # GET /boats/1.json
  def confirmation
    @info = NominationType.where(["code = ?", "Pmpdp05"]).first
  end

  # GET /boats/new
  def new
    @nomination = Pmpdp05.new
    @info = NominationType.where(["code = ?", "Pmpdp05"]).first
    @award_options = AwardOption.joins(:nomination_type).where("code  = ?", "Pmpdp05").pluck(:name,:id)
    @callback = "/pmpdp05s/?#no-back"
    begin
  ...
  # POST /boats.json
  def create
    @nomination = Pmpdp05.new(nomination_params)
    @info = NominationType.where(["code = ?", "Pmpdp05"]).first
    ...
    respond_to do |format|
      if @nomination.save
        ...
        format.html { redirect_to '/pmpdp05s/confirmation', :layout => "nomination_form", notice: 'Nomination was successfully created.' }
  ...
  private
    ...
    def set_nomination
      @pmpdp05 = Pmpdp05.find(params[:id])
    end
    ...
    def nomination_type
        Nomination.nomination_types.include?(params[:type]) ? params[:type] : "Pmpdp05"
    end
