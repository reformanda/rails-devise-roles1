class DecpsController < ApplicationController
  ...
  # POST /boats.json
  def create
    @nomination = Decp.new(nomination_params)
    @info = NominationType.where(["code = ?", "Decp"]).first
    ...
    respond_to do |format|
      if @nomination.save
        ...
        format.html { redirect_to '/decps/confirmation', :layout => "nomination_form", notice: 'Nomination was successfully created.' }
  ...
  private
    ...
    def nomination_type
        Nomination.nomination_types.include?(params[:type]) ? params[:type] : "Decp"
    end
