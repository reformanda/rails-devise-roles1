class AspsController < ApplicationController
  ...
  # POST /boats.json
  def create
    @nomination = Asp.new(nomination_params)
    @info = NominationType.where(["code = ?", "Asp"]).first
    ...
    respond_to do |format|
      if @nomination.save
        ...
        format.html { redirect_to '/asps/confirmation', :layout => "nomination_form", notice: 'Nomination was successfully created.' }
  ...
  private
    ...
    def nomination_type
        Nomination.nomination_types.include?(params[:type]) ? params[:type] : "Asp"
    end
