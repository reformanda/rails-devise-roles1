class Pmpdo05sController < ApplicationController
  ...
  # POST /boats.json
  def create
    @nomination = Pmpdo05.new(nomination_params)
    @info = NominationType.where(["code = ?", "Pmpdo05"]).first
  ...
  private
    ...
    def nomination_type
        Nomination.nomination_types.include?(params[:type]) ? params[:type] : "Pmpdo05"
    end
